package com.howtodoinjava.example.employee.api;

import com.howtodoinjava.example.employee.model.Employee;

import java.util.*;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.test.context.junit4.SpringRunner;

import static org.junit.Assert.assertEquals;

@RunWith(SpringRunner.class)
@SpringBootTest
public class FindEmployeeDetailsApiControllerIntegrationTest {

    @Autowired
    private FindEmployeeDetailsApi api;

    @Test
    public void getEmployeeDetailsTest() throws Exception {
        Long employeeId = 789L;
        ResponseEntity<Employee> responseEntity = api.getEmployeeDetails(employeeId);
        assertEquals(HttpStatus.NOT_IMPLEMENTED, responseEntity.getStatusCode());
    }

}
